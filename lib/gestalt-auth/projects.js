...
const PROJECT_COLUMNS = `
  ...
  body, body_pt, stack, stack_pt, role, role_pt, client, client_pt,
  cover_url, external_url, status, sort_order, updated_at
`.replace(/\s+/g, ' ').trim()
...
export function localizeProject(row, locale = 'en') {
  ...
  return {
    ...
    stack: (usePt && row.stack_pt) || row.stack || null,
    role: (usePt && row.role_pt) || row.role || null,
    client: (usePt && row.client_pt) || row.client || null,
