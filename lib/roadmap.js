...
/**
 * Esteira review priority (partials/uc-esteira.md): unconfirmed specs are
 * the next thing the owner needs to review, so they sort ahead of both
 * already-confirmed specs and roadmap entries with no spec card at all.
 */
function esteiraReviewRank(spec) {
  if (!spec) return 2
  const reviewStatus = spec.metadata?.esteira?.review_status
  return !reviewStatus || reviewStatus === 'unchecked' ? 0 : 1
}

/** Add DB specs that have no roadmap quests yet (e.g. designing products with documented UCs). */
export function mergeUseCaseProgressWithSpecs(ucProgress, useCases, productFilter = 'all') {
  ...
  for (const item of useCases) {
    const gestaltCode = SPEC_TO_GESTALT[item.productCode] ?? item.productCode?.toUpperCase?.()
    if (!gestaltCode) continue
    const key = ucProgressKey({ uc: item.ucNumber, productCode: gestaltCode }, productFilter)
    specByKey.set(key, item)
  }

  return merged.sort((a, b) => {
    const rankA = esteiraReviewRank(specByKey.get(ucProgressKey(a, productFilter)))
    const rankB = esteiraReviewRank(specByKey.get(ucProgressKey(b, productFilter)))
